...
class DiffMatchPatch {
      ...
      switch (diffs[pointer].operation) {
        ...
        case Operation.equal:
          ...
          if (count_delete >= 1 && count_insert >= 1) {
            ...
            final subDiff =
                diff_main(text_delete.toString(), text_insert.toString(), false, deadline);
  ...
  String _diff_linesToCharsMunge(
      String text, List<String> lineArray, Map<String, int> lineHash, int maxLines) {
    var lineStart = 0;
    var lineEnd = -1;
    String line;
      ...
      if (diffs[pointer - 1].operation == Operation.delete &&
        ...
        if (overlap_length1 >= overlap_length2) {
          ...
          if (overlap_length2 >= deletion.length / 2 || overlap_length2 >= insertion.length / 2) {
            ...
            diffs[pointer - 1] =
                Diff(Operation.insert, insertion.substring(0, insertion.length - overlap_length2));
      ...
      if (diffs[pointer - 1].operation == Operation.equal &&
        ...
        if (diffs[pointer].text!.endsWith(diffs[pointer - 1].text!)) {
          ...
          diffs[pointer].text = diffs[pointer - 1].text! +
              diffs[pointer]
                  .text!
                  .substring(0, diffs[pointer].text!.length - diffs[pointer - 1].text!.length);
  ...
  void patch_splitMax(List<Patch> patches) {
    ...
    for (var x = 0; x < patches.length; x++) {
      ...
      while (bigpatch.diffs!.isNotEmpty) {
        ...
        while (bigpatch.diffs!.isNotEmpty && patch.length1 < patch_size - Patch_Margin) {
          ...
          if (diff_type == Operation.insert) {
            ...
            diff_text = diff_text!
                .substring(0, min(diff_text.length, patch_size - patch.length1 - Patch_Margin));
            patch.length1 += diff_text.length;
            start1 = start1! + diff_text.length;
        ...
        if (postcontext.isNotEmpty) {
          ...
          if (patch.diffs!.isNotEmpty && patch.diffs!.last.operation == Operation.equal) {
