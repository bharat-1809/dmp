...
class Expect {
  ...
  static void isNull(actual, String msg) {
    ...
    throw Exception('Expect.isNull(actual: <$actual>$msg) fails.');
  }

  static void isTrue(var actual, String msg) {
    if (identical(actual, true)) return;
    throw Exception('Expect.isTrue($actual, $msg) fails.');
  }

  static void throws(void Function() f, String msg) {
    try {
      f();
    } catch (e) {
      return;
    }
    throw Exception('Expect.throws($msg) fails');
  }

  static void listEquals(List expected, List actual, String msg) {
    var n = (expected.length < actual.length) ? expected.length : actual.length;
    for (var i = 0; i < n; i++) {
      if (expected[i] != actual[i]) {
        throw Exception('Expect.listEquals(at index $i, '
    ...
    if (expected.length != actual.length) {
      throw Exception('Expect.listEquals(list length, '
  ...
  static void mapEquals(Map expected, Map actual, String msg) {
    for (var k in actual.keys) {
      if (!expected.containsKey(k)) {
        throw Exception('Expect.mapEquals(unexpected key <$k> found '
            'expected: <$expected>, actual: <$actual> $msg) fails');
      }
    }
    for (var k in expected.keys) {
      if (!actual.containsKey(k)) {
        throw Exception('Expect.mapEquals(key <$k> not found '
            'expected: <$expected>, actual: <$actual> $msg) fails');
      }
...
void testDiffCommonPrefix() {
  ...
  Expect.equals(
      0, dmp.diff_commonPrefix('abc', 'xyz'), 'diff_commonPrefix: Null case.');

  Expect.equals(4, dmp.diff_commonPrefix('1234abcdef', '1234xyz'),
      'diff_commonPrefix: Non-null case.');

  Expect.equals(4, dmp.diff_commonPrefix('1234', '1234xyz'),
...
void testDiffCommonSuffix() {
  ...
  Expect.equals(
      0, dmp.diff_commonSuffix('abc', 'xyz'), 'diff_commonSuffix: Null case.');

  Expect.equals(4, dmp.diff_commonSuffix('abcdef1234', 'xyz1234'),
      'diff_commonSuffix: Non-null case.');

  Expect.equals(4, dmp.diff_commonSuffix('1234', 'xyz1234'),
      'diff_commonSuffix: Whole case.');
}

void testDiffCommonOverlap() {
  // Detect any suffix/prefix overlap.
  Expect.equals(0, dmp.test_diff_commonOverlap('', 'abcd'),
      'diff_commonOverlap: Null case.');

  Expect.equals(3, dmp.test_diff_commonOverlap('abc', 'abcd'),
      'diff_commonOverlap: Whole case.');

  Expect.equals(0, dmp.test_diff_commonOverlap('123456', 'abcd'),
      'diff_commonOverlap: No overlap.');

  Expect.equals(3, dmp.test_diff_commonOverlap('123456xxx', 'xxxabcd'),
      'diff_commonOverlap: Overlap.');

  // Some overly clever languages (C#) may treat ligatures as equal to their
  // component letters.  E.g. U+FB01 == 'fi'
  Expect.equals(0, dmp.test_diff_commonOverlap('fi', '\ufb01i'),
...
void testDiffHalfmatch() {
  ...
  Expect.isNull(dmp.test_diff_halfMatch('1234567890', 'abcdef'),
      'diff_halfMatch: No match #1.');

  Expect.isNull(
      dmp.test_diff_halfMatch('12345', '23'), 'diff_halfMatch: No match #2.');

  Expect.listEquals(
      ['12', '90', 'a', 'z', '345678'],
      dmp.test_diff_halfMatch('1234567890', 'a345678z')!,
      'diff_halfMatch: Single Match #1.');

  Expect.listEquals(
      ['a', 'z', '12', '90', '345678'],
      dmp.test_diff_halfMatch('a345678z', '1234567890')!,
      'diff_halfMatch: Single Match #2.');

  Expect.listEquals(
      ['abc', 'z', '1234', '0', '56789'],
      dmp.test_diff_halfMatch('abc56789z', '1234567890')!,
      'diff_halfMatch: Single Match #3.');

  Expect.listEquals(
      ['a', 'xyz', '1', '7890', '23456'],
      dmp.test_diff_halfMatch('a23456xyz', '1234567890')!,
  ...
  Expect.listEquals(
      ...
      dmp.test_diff_halfMatch('qHilloHelloHew', 'xHelloHeHulloy')!,
      'diff_halfMatch: Non-optimal halfmatch.');

  dmp.Diff_Timeout = 0.0;
  Expect.isNull(dmp.test_diff_halfMatch('qHilloHelloHew', 'xHelloHeHulloy'),
      'diff_halfMatch: Optimal no halfmatch.');
}

void testDiffLinesToChars() {
  void assertLinesToCharsResultEquals(
      Map<String, dynamic> a, Map<String, dynamic> b, String error_msg) {
  ...
  assertLinesToCharsResultEquals(
      ...
      dmp.test_diff_linesToChars(lines, ''),
      'diff_linesToChars: More than 256.');
}

void testDiffCharsToLines() {
  // First check that Diff equality works.
  Expect.isTrue(Diff(Operation.equal, 'a') == Diff(Operation.equal, 'a'),
      'diff_charsToLines: Equality #1.');

  Expect.equals(Diff(Operation.equal, 'a'), Diff(Operation.equal, 'a'),
  ...
  Expect.listEquals([Diff(Operation.delete, lines)], diffs,
...
void testDiffCleanupMerge() {
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([Diff(Operation.equal, 'abc')], diffs,
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([Diff(Operation.delete, 'abc')], diffs,
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([Diff(Operation.insert, 'abc')], diffs,
  ...
  ], diffs, 'diff_cleanupMerge: Merge interweave.');
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([Diff(Operation.insert, 'ab'), Diff(Operation.equal, 'ac')],
      diffs, 'diff_cleanupMerge: Slide edit left.');
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([Diff(Operation.equal, 'ca'), Diff(Operation.insert, 'ba')],
      diffs, 'diff_cleanupMerge: Slide edit right.');
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([Diff(Operation.insert, 'a'), Diff(Operation.equal, 'bc')],
      diffs, 'diff_cleanupMerge: Empty merge.');
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([Diff(Operation.insert, 'a'), Diff(Operation.equal, 'b')],
      diffs, 'diff_cleanupMerge: Empty equality.');
}
...
void testDiffCleanupSemanticLossless() {
  ...
  dmp.test_diff_cleanupSemanticLossless(diffs);
  Expect.listEquals([Diff(Operation.delete, 'a'), Diff(Operation.equal, 'aax')],
      diffs, 'diff_cleanupSemanticLossless: Hitting the start.');
  ...
  dmp.test_diff_cleanupSemanticLossless(diffs);
  Expect.listEquals([Diff(Operation.equal, 'xaa'), Diff(Operation.delete, 'a')],
      diffs, 'diff_cleanupSemanticLossless: Hitting the end.');
...
void testDiffCleanupSemantic() {
  ...
  ], diffs, 'diff_cleanupSemantic: Overlap elimination.');
  ...
  Expect.listEquals([
    ...
    Diff(Operation.equal, 'xxx'),
    Diff(Operation.delete, 'abc')
  ], diffs, 'diff_cleanupSemantic: Reverse overlap elimination.');
...
void testDiffCleanupEfficiency() {
  ...
  dmp.diff_cleanupEfficiency(diffs);
  Expect.listEquals(
      [Diff(Operation.delete, 'abxyzcd'), Diff(Operation.insert, '12xyz34')],
      diffs,
  ...
  dmp.diff_cleanupEfficiency(diffs);
  Expect.listEquals(
      [Diff(Operation.delete, 'xcd'), Diff(Operation.insert, '12x34')],
      diffs,
  ...
  dmp.diff_cleanupEfficiency(diffs);
  Expect.listEquals(
      [Diff(Operation.delete, 'abxyzcd'), Diff(Operation.insert, '12xy34z56')],
      diffs,
  ...
  dmp.diff_cleanupEfficiency(diffs);
  Expect.listEquals(
      [Diff(Operation.delete, 'abwxyzcd'), Diff(Operation.insert, '12wxyz34')],
      diffs,
...
void testDiffPrettyHtml() {
  ...
  var diffs = <Diff>[
    Diff(Operation.equal, 'a\n'),
    Diff(Operation.delete, '<B>b</B>'),
    Diff(Operation.insert, 'c&d')
  ];
  Expect.equals(
...
void testDiffDelta() {
  ...
  Expect.equals(
      '=4\t-1\t+ed\t=6\t-3\t+a\t=5\t+old dog', delta, 'diff_toDelta:');

  // Convert delta string into a diff.
  Expect.listEquals(
      diffs, dmp.diff_fromDelta(text1, delta), 'diff_fromDelta: Normal.');

  // Generates error (19 < 20).
  Expect.throws(() => dmp.diff_fromDelta(text1 + 'x', delta),
      'diff_fromDelta: Too long.');

  // Generates error (19 > 18).
  Expect.throws(() => dmp.diff_fromDelta(text1.substring(1), delta),
      'diff_fromDelta: Too short.');

  // Generates error (%c3%xy invalid Unicode).
  Expect.throws(() => dmp.diff_fromDelta('', '+%c3%xy'),
      'diff_fromDelta: Invalid character.');

  // Test deltas with special characters.
  diffs = [
    Diff(Operation.equal, '\u0680 \x00 \t %'),
    Diff(Operation.delete, '\u0681 \x01 \n ^'),
  ...
  Expect.listEquals(
      diffs, dmp.diff_fromDelta(text1, delta), 'diff_fromDelta: Unicode.');
  ...
  Expect.equals('A-Z a-z 0-9 - _ . ! ~ * \' ( ) ; / ? : @ & = + \$ , # ', text2,
      'diff_text2: Unchanged characters.');

  delta = dmp.diff_toDelta(diffs);
  Expect.equals('+A-Z a-z 0-9 - _ . ! ~ * \' ( ) ; / ? : @ & = + \$ , # ',
      delta, 'diff_toDelta: Unchanged characters.');

  // Convert delta string into a diff.
  Expect.listEquals(diffs, dmp.diff_fromDelta('', delta),
  ...
  diffs = [Diff(Operation.insert, a)];
  delta = dmp.diff_toDelta(diffs);
  Expect.equals('+' + a, delta, 'diff_toDelta: 160kb string.');

  // Convert delta string into a diff.
  Expect.listEquals(
      diffs, dmp.diff_fromDelta('', delta), 'diff_fromDelta: 160kb string.');
}
...
void testDiffXIndex() {
  ...
  Expect.equals(
      5, dmp.diff_xIndex(diffs, 2), 'diff_xIndex: Translation on equality.');

  diffs = [
    Diff(Operation.equal, 'a'),
    Diff(Operation.delete, '1234'),
    Diff(Operation.equal, 'xyz')
  ];
  Expect.equals(
      1, dmp.diff_xIndex(diffs, 3), 'diff_xIndex: Translation on deletion.');
}
...
void testDiffLevenshtein() {
  ...
  Expect.equals(
      4, dmp.diff_levenshtein(diffs), 'Levenshtein with trailing equality.');
  ...
  Expect.equals(
      4, dmp.diff_levenshtein(diffs), 'Levenshtein with leading equality.');
  ...
  Expect.equals(
      7, dmp.diff_levenshtein(diffs), 'Levenshtein with middle equality.');
}
void testDiffBisect() {
  // Normal.
  var a = 'cat';
  ...
  Expect.listEquals(
      diffs, dmp.test_diff_bisect(a, b, deadline), 'diff_bisect: Normal.');

  // Timeout.
  diffs = [Diff(Operation.delete, 'cat'), Diff(Operation.insert, 'map')];
  // Set deadline to one year ago.
  deadline = DateTime.now().subtract(Duration(days: 365));
  Expect.listEquals(
      diffs, dmp.test_diff_bisect(a, b, deadline), 'diff_bisect: Timeout.');
}
...
void testDiffMain() {
  ...
  var diffs = <Diff>[];
  Expect.listEquals(
      diffs, dmp.diff_main('', '', false), 'diff_main: Null case.');

  diffs = [Diff(Operation.equal, 'abc')];
  Expect.listEquals(
      diffs, dmp.diff_main('abc', 'abc', false), 'diff_main: Equality.');
  ...
  Expect.listEquals(diffs, dmp.diff_main('abc', 'ab123c', false),
  ...
  Expect.listEquals(diffs, dmp.diff_main('a123bc', 'abc', false),
  ...
  Expect.listEquals(diffs, dmp.diff_main('abc', 'a123b456c', false),
  ...
  Expect.listEquals(diffs, dmp.diff_main('a123b456c', 'abc', false),
  ...
  Expect.listEquals(
      diffs, dmp.diff_main('a', 'b', false), 'diff_main: Simple case #1.');
  ...
  Expect.listEquals(diffs, dmp.diff_main('ax\t', '\u0680x\000', false),
  ...
  Expect.listEquals(
      diffs, dmp.diff_main('1ayb2', 'abxab', false), 'diff_main: Overlap #1.');
  ...
  Expect.listEquals(diffs, dmp.diff_main('abcy', 'xaxcxabc', false),
      'diff_main: Overlap #2.');
  ...
  Expect.isTrue(
      dmp.Diff_Timeout * 2 > elapsedSeconds, 'diff_main: Timeout max.');
  dmp.Diff_Timeout = 0.0;
  ...
  a = '1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n';
  b = 'abcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\nabcdefghij\n';
  Expect.listEquals(dmp.diff_main(a, b, true), dmp.diff_main(a, b, false),
  ...
  a = '1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n1234567890\n';
  b = 'abcdefghij\n1234567890\n1234567890\n1234567890\nabcdefghij\n1234567890\n1234567890\n1234567890\nabcdefghij\n1234567890\n1234567890\n1234567890\nabcdefghij\n';
  var texts_linemode = _diff_rebuildtexts(dmp.diff_main(a, b, true));
  var texts_textmode = _diff_rebuildtexts(dmp.diff_main(a, b, false));
  Expect.listEquals(
      texts_textmode, texts_linemode, 'diff_main: Overlap line-mode.');
...
void testMatchAlphabet() {
  ...
  Expect.mapEquals(
      bitmask, dmp.test_match_alphabet('abc'), 'match_alphabet: Unique.');

  bitmask = {'a': 37, 'b': 18, 'c': 8};
  Expect.mapEquals(bitmask, dmp.test_match_alphabet('abcaba'),
...
void testMatchBitap() {
  ...
  Expect.equals(5, dmp.test_match_bitap('abcdefghijk', 'fgh', 5),
      'match_bitap: Exact match #1.');

  Expect.equals(5, dmp.test_match_bitap('abcdefghijk', 'fgh', 0),
      'match_bitap: Exact match #2.');

  Expect.equals(4, dmp.test_match_bitap('abcdefghijk', 'efxhi', 0),
      'match_bitap: Fuzzy match #1.');

  Expect.equals(2, dmp.test_match_bitap('abcdefghijk', 'cdefxyhijk', 5),
      'match_bitap: Fuzzy match #2.');

  Expect.equals(-1, dmp.test_match_bitap('abcdefghijk', 'bxy', 1),
      'match_bitap: Fuzzy match #3.');

  Expect.equals(2, dmp.test_match_bitap('123456789xx0', '3456789x0', 2),
      'match_bitap: Overflow.');

  Expect.equals(0, dmp.test_match_bitap('abcdef', 'xxabc', 4),
      'match_bitap: Before start match.');

  Expect.equals(3, dmp.test_match_bitap('abcdef', 'defyy', 4),
      'match_bitap: Beyond end match.');

  Expect.equals(0, dmp.test_match_bitap('abcdef', 'xabcdefy', 0),
      'match_bitap: Oversized pattern.');

  dmp.Match_Threshold = 0.4;
  Expect.equals(4, dmp.test_match_bitap('abcdefghijk', 'efxyhi', 1),
      'match_bitap: Threshold #1.');

  dmp.Match_Threshold = 0.3;
  Expect.equals(-1, dmp.test_match_bitap('abcdefghijk', 'efxyhi', 1),
      'match_bitap: Threshold #2.');

  dmp.Match_Threshold = 0.0;
  Expect.equals(1, dmp.test_match_bitap('abcdefghijk', 'bcdef', 1),
      'match_bitap: Threshold #3.');

  dmp.Match_Threshold = 0.5;
  Expect.equals(0, dmp.test_match_bitap('abcdexyzabcde', 'abccde', 3),
      'match_bitap: Multiple select #1.');

  Expect.equals(8, dmp.test_match_bitap('abcdexyzabcde', 'abccde', 5),
      'match_bitap: Multiple select #2.');

  dmp.Match_Distance = 10; // Strict location.
  Expect.equals(
      -1,
      dmp.test_match_bitap('abcdefghijklmnopqrstuvwxyz', 'abcdefg', 24),
      'match_bitap: Distance test #1.');

  Expect.equals(
      0,
      dmp.test_match_bitap('abcdefghijklmnopqrstuvwxyz', 'abcdxxefg', 1),
...
void testMatchMain() {
  // Full match.
  Expect.equals(
      0, dmp.match_main('abcdef', 'abcdef', 1000), 'match_main: Equality.');

  Expect.equals(-1, dmp.match_main('', 'abcdef', 1), 'match_main: Null text.');

  Expect.equals(
      3, dmp.match_main('abcdef', '', 3), 'match_main: Null pattern.');

  Expect.equals(
      3, dmp.match_main('abcdef', 'de', 3), 'match_main: Exact match.');

  Expect.equals(
      3, dmp.match_main('abcdef', 'defy', 4), 'match_main: Beyond end match.');

  Expect.equals(0, dmp.match_main('abcdef', 'abcdefy', 0),
  ...
  // Test null inputs.
  Expect.throws(
      () => dmp.match_main(null, null, 0), 'match_main: Null inputs.');
}
...
void testPatchFromText() {
  ...
  var strp =
      '@@ -21,18 +22,17 @@\n jump\n-s\n+ed\n  over \n-the\n+a\n %0Alaz\n';
  Expect.equals(
      strp, dmp.patch_fromText(strp)[0].toString(), 'patch_fromText: #1.');
...
void testPatchMake() {
  ...
  Expect.equals(expectedPatch, dmp.patch_toText(patches),
  ...
  Expect.equals(
      expectedPatch, dmp.patch_toText(patches), 'patch_make: Diff input.');
  ...
  diffs = [
    Diff(Operation.delete, '`1234567890-=[]\\;\',./'),
    Diff(Operation.insert, '~!@#\$%^&*()_+{}|:"<>?')
  ];
  Expect.listEquals(
      diffs,
      dmp
...
void testPatchApply() {
  ...
  Expect.equals('That quick brown fox jumped over a lazy dog.\ttrue\ttrue',
      resultStr, 'patch_apply: Exact match.');

  results = dmp.patch_apply(
      patches, 'The quick red rabbit jumps over the tired tiger.');
  boolArray = results[1];
  resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
  Expect.equals('That quick red rabbit jumped over a tired tiger.\ttrue\ttrue',
      resultStr, 'patch_apply: Partial match.');

  results = dmp.patch_apply(
      patches, 'I am the very model of a modern major general.');
  boolArray = results[1];
  resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
  Expect.equals('I am the very model of a modern major general.\tfalse\tfalse',
      resultStr, 'patch_apply: Failed match.');
  ...
  resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
  Expect.equals(
      'xabcy\ttrue\ttrue', resultStr, 'patch_apply: Big delete, small change.');
  ...
  resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
  Expect.equals(
      'xabcy\ttrue\ttrue', resultStr, 'patch_apply: Big delete, big change 2.');
  dmp.Patch_DeleteThreshold = 0.5;
  ...
  Expect.equals(
      patchStr, dmp.patch_toText(patches), 'patch_apply: No side effects.');

  patches =
      dmp.patch_make('The quick brown fox jumps over the lazy dog.', 'Woof');
  patchStr = dmp.patch_toText(patches);
  dmp.patch_apply(patches, 'The quick brown fox jumps over the lazy dog.');
  Expect.equals(patchStr, dmp.patch_toText(patches),
  ...
  resultStr = '${results[0]}\t${boolArray[0]}';
  Expect.equals(
      'XtestY\ttrue', resultStr, 'patch_apply: Near edge exact match.');
